...
- HTTPS-проверки через Tachyon (sing-box или Steer), HomeProxy, Forkop/Podkop и tproxy.
...
- Отображение fakeip, времени DNS/TCP/TLS/HTTP и выбранного outbound через Clash API при sing-box. При Steer показываются состояние движка и план правил; фактический outbound без телеметрии движка остаётся неподтверждённым.
...
- Расширяемая панель **«Фиксы Forkop»** показывается только при установленном Forkop и отсутствии Tachyon: новые исправления добавляются в whitelist-реестр backend и не позволяют запускать произвольные команды из браузера.
...
- Под-вкладка **Подбор стратегии Zapret** (скрыта при установленном Tachyon): находит установленный Zapret/Zapret2 и штатный `blockcheck`, выполняет автоподбор TLS 1.3/TLS 1.2/QUIC по модели Slipstream либо проверяет отдельный каталог готовых профилей. Пользователь выбирает YouTube, Discord, Telegram и собственные доменные сервисы; последние результаты сохраняются на роутере, полностью нерабочие варианты свёрнуты.
- Режим **Под формат Forkop** удаляет только служебные `qnum`, `fwmark`, hostlist/ipset и process-management аргументы, сохраняет полные TCP/UDP-профили и границы `--new`, затем выдаёт одну строку для поля `NFQWS Strategy` либо `NFQWS2 Strategy` в секции Forkop.
...
После завершения запуска модуль хранит до десяти кратких результатов в `/var/run/sing-box-service-check/history.json`. Это tmpfs: история не изнашивает flash и очищается при перезагрузке. В интерфейсе видно, какие сервисы изменили статус, outbound или время ответа. Там же можно очистить историю.
...
Во вкладке **VPN → Подбор стратегии Zapret** выбирается установленный движок Zapret или Zapret2 и один из двух режимов. **Автоподбор** запускает найденный штатный `blockcheck`/`blockcheck2` отдельными проходами для TLS 1.3, TLS 1.2 и QUIC, отдаёт приоритет общей TLS-стратегии и собирает несколько полных вариантов. **Готовые профили** оставлены отдельной вкладкой и проверяют 4, 8 или весь локальный каталог. Интерфейс показывает точные пути к корню установки, `nfqws`/`nfqws2`, `blockcheck` и каталогу.

Перед запуском модуль запоминает активные Forkop, Tachyon, Podkop и самостоятельные сервисы Zapret, останавливает их и дважды проверяет фактическое состояние. После завершения, отмены или таймаута запускаются только те сервисы, которые работали до теста. Установленные конфиги Forkop и Zapret не изменяются.

Во время поиска показываются текущий протокол, номер прохода, время и таймаут, сколько стратегий уже проверил `blockcheck`, сколько найдено и какой вариант проверяется сейчас. Затем каждый собранный или готовый профиль запускается найденным локальным движком в отдельной временной очереди `nftables`, ограниченной IP-адресами выбранных HTTPS endpoint’ов. В блоке **«Какие сервисы проверяем»** встроенные YouTube, Discord и Telegram включаются независимо; в сворачиваемом редакторе можно добавить собственный сервис с 1–8 доменами. Выбор и пользовательский список хранятся в `/etc/sing-box-service-check/zapret_services.json`.

Если выбран Discord, к каждому варианту добавляется отдельный сигнатурный блок Voice IP Discovery + STUN, и интерфейс показывает, загрузил ли его установленный движок. Когда Discord не выбран, этот UDP-блок не добавляется и не влияет на рейтинг. Полноценный голосовой сеанс нельзя воспроизвести без авторизованного Discord Gateway-сеанса и выданного им динамического voice endpoint, поэтому HTTPS-балл и статус загрузки голосового блока намеренно показаны раздельно.

Результаты сгруппированы по полной и частичной работоспособности, а нерабочие варианты находятся в отдельном свёрнутом списке. У каждого варианта показана фактическая статистика выбранных сервисов, название и источник. Последний результат сохраняется отдельно для каждой пары движка и режима в `/etc/sing-box-service-check/zapret_strategy_results.json`, автоматически возвращается после открытия страницы и удаляется отдельной кнопкой. Временные логи перебора остаются в `/var/run` и не записываются во flash.

Переключатель **«Под формат Forkop»** выдаёт одну готовую строку без оболочки `NFQWS_OPT=...`: для действия **Zapret** её нужно вставить в поле **NFQWS Strategy**, для действия **Zapret2** — в **NFQWS2 Strategy**. В строке убраны параметры очереди, `fwmark`, управление процессом и hostlist/ipset, но сохранены полные TCP/UDP-фильтры, payload, нужный голосовой блок и границы `--new`.
...
Для этой проверки нужен собственный Gemini API-ключ. Он задаётся в раскрытой карточке, хранится на роутере в `/etc/sing-box-service-check/gemini_api_key` с правами `0600` и не включается в пакет. Без ключа геопроверка пропускается, а остальные цели Gemini продолжают проверяться как раньше.
...
```text
/etc/sing-box-service-check/profiles.json
```

Этот файл имеет приоритет над встроенным `/usr/share/sing-box-service-check/profiles.json`.
