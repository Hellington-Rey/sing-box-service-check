...
- Под-вкладка **Подбор стратегии Zapret**: находит установленный Zapret/Zapret2 и штатный `blockcheck`, выполняет автоподбор TLS 1.3/TLS 1.2/QUIC по модели Slipstream либо проверяет отдельный каталог готовых профилей. Каждый вариант получает оценки YouTube, Discord HTTPS и Telegram; полностью нерабочие результаты свёрнуты.
...
Во время поиска показываются текущий протокол, номер прохода, время и таймаут, сколько стратегий уже проверил `blockcheck`, сколько найдено и какой вариант проверяется сейчас. Затем каждый собранный или готовый профиль запускается найденным локальным движком в отдельной временной очереди `nftables`, ограниченной IP-адресами 12 HTTPS endpoint’ов: по четыре для YouTube, Discord и Telegram. Для Discord Voice к каждому варианту добавляется отдельный сигнатурный блок IP Discovery + STUN; интерфейс отдельно показывает, загрузил ли его установленный движок. Полноценный голосовой сеанс нельзя воспроизвести без авторизованного Discord Gateway-сеанса и выданного им динамического voice endpoint, поэтому HTTPS-балл и статус загрузки голосового блока намеренно показаны раздельно.

Результаты сгруппированы по полной и частичной работоспособности, а нерабочие варианты находятся в отдельном свёрнутом списке. У каждого варианта показано число успешных проверок YouTube, Discord HTTPS и Telegram, название и источник. Переключатель **«Под формат Forkop»** выдаёт одну готовую строку без оболочки `NFQWS_OPT=...`: для действия **Zapret** её нужно вставить в поле **NFQWS Strategy**, для действия **Zapret2** — в **NFQWS2 Strategy**. В строке убраны параметры очереди, `fwmark`, управление процессом и hostlist/ipset, но сохранены полные TCP/UDP-фильтры, payload, голосовой блок и границы `--new`.
