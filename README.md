...
Во время поиска показываются текущий протокол, номер прохода, время и таймаут, сколько стратегий уже проверил `blockcheck`, сколько найдено и какой вариант проверяется сейчас. Затем каждый собранный или готовый профиль запускается найденным локальным движком в отдельной временной очереди `nftables`, ограниченной IP-адресами выбранных HTTPS endpoint’ов. В блоке **«Какие сервисы проверяем»** встроенные YouTube, Discord и Telegram включаются независимо; в сворачиваемом редакторе можно добавить собственный сервис с 1–8 доменами. Выбор и пользовательский список хранятся в `/etc/forkop-servicecheck/zapret_services.json`.
...
Результаты сгруппированы по полной и частичной работоспособности, а нерабочие варианты находятся в отдельном свёрнутом списке. У каждого варианта показана фактическая статистика выбранных сервисов, название и источник. Последний результат сохраняется отдельно для каждой пары движка и режима в `/etc/forkop-servicecheck/zapret_strategy_results.json`, автоматически возвращается после открытия страницы и удаляется отдельной кнопкой. Временные логи перебора остаются в `/var/run` и не записываются во flash.
