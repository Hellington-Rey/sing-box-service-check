...
- Под-вкладка **Подбор стратегии Zapret**: находит корень и бинарник установленного Zapret/Zapret2, проверяет локальный каталог готовых составных профилей по четырём HTTPS endpoint’ам YouTube, Discord и Telegram, ранжирует результаты и отдельно сворачивает полностью нерабочие варианты.
...
```sh
opkg install luci-app-forkop-servicecheck_1.12.3-r1_all.ipk
```
...
Во вкладке **VPN → Подбор стратегии Zapret** выбирается установленный движок Zapret или Zapret2 и набор из 4, 8 или всех готовых профилей. Интерфейс показывает точные пути к корню установки, `nfqws`/`nfqws2` и штатному `blockcheck`. Перед запуском модуль запоминает активные Forkop, Tachyon, Podkop и самостоятельные сервисы Zapret, останавливает их и дважды проверяет фактическое состояние. После завершения, отмены или таймаута запускаются только те сервисы, которые работали до теста.

Каждый готовый профиль запускается найденным локальным движком в отдельной временной очереди `nftables`, ограниченной IP-адресами 12 целевых endpoint’ов. HTTPS-проверки одного профиля идут параллельно; установленная конфигурация Zapret и конфигурация Forkop не переписываются. Штатный `blockcheck` отображается только для диагностики расположения установки и не запускается, поэтому его дополнительные DNS-домены в тест не попадают. Во время выполнения показываются прошедшее время, текущий профиль, число выполненных запросов, полные/частичные/нерабочие результаты, DNS-готовность и последняя цель.

Результаты сгруппированы по полной и частичной работоспособности, а нерабочие варианты находятся в отдельном свёрнутом списке. У каждого варианта показано число успешных проверок YouTube, Discord и Telegram, название и источник готового профиля. Переключатель **«Под формат Forkop»** выдаёт одну готовую строку без оболочки `NFQWS_OPT=...`: для действия **Zapret** её нужно вставить в поле **NFQWS Strategy**, для действия **Zapret2** — в **NFQWS2 Strategy**. В строке убраны параметры очереди, `fwmark`, управление процессом и hostlist/ipset, но сохранены полные TCP/UDP-фильтры, payload и границы `--new`.
