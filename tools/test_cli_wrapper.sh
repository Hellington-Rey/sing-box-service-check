...
[ "$(run_wrapper capabilities fast | tail -n 2 | head -n 1)" = "capabilities" ]
[ "$(run_wrapper capabilities fast | tail -n 1)" = "fast" ]
...
[ "$(run_wrapper zapret-settings | tail -n 1)" = "zapret-settings" ]
ZAPRET_SERVICES='{"services":[],"selected":["youtube"]}'
ZAPRET_SERVICES_OUTPUT="$(run_wrapper zapret-services-save "$ZAPRET_SERVICES")"
[ "$(printf '%s\n' "$ZAPRET_SERVICES_OUTPUT" | tail -n 2 | head -n 1)" = "zapret-services-save" ]
[ "$(printf '%s\n' "$ZAPRET_SERVICES_OUTPUT" | tail -n 1)" = "$ZAPRET_SERVICES" ]
ZAPRET_SELECTION='["youtube","discord"]'
ZAPRET_START_OUTPUT="$(run_wrapper zapret-start zapret2 ready force "$ZAPRET_SELECTION")"
[ "$(printf '%s\n' "$ZAPRET_START_OUTPUT" | tail -n 5 | head -n 1)" = "zapret-start" ]
[ "$(printf '%s\n' "$ZAPRET_START_OUTPUT" | tail -n 4 | head -n 1)" = "zapret2" ]
[ "$(printf '%s\n' "$ZAPRET_START_OUTPUT" | tail -n 3 | head -n 1)" = "ready" ]
[ "$(printf '%s\n' "$ZAPRET_START_OUTPUT" | tail -n 2 | head -n 1)" = "force" ]
[ "$(printf '%s\n' "$ZAPRET_START_OUTPUT" | tail -n 1)" = "$ZAPRET_SELECTION" ]
...
ZAPRET_CLEAR_OUTPUT="$(run_wrapper zapret-results-clear zapret2 ready)"
[ "$(printf '%s\n' "$ZAPRET_CLEAR_OUTPUT" | tail -n 3 | head -n 1)" = "zapret-results-clear" ]
[ "$(printf '%s\n' "$ZAPRET_CLEAR_OUTPUT" | tail -n 2 | head -n 1)" = "zapret2" ]
[ "$(printf '%s\n' "$ZAPRET_CLEAR_OUTPUT" | tail -n 1)" = "ready" ]
